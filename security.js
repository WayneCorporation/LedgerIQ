...
function initSecurity(db,h){
 ...
 async function handlePublic(req,res,url){
  ...
  if(req.method==='POST'&&url.pathname==='/api/auth/password/reset'){const d=await body(req),password=String(d.password||''),row=db.prepare(`SELECT * FROM auth_tokens WHERE token_hash=? AND type='password_reset' AND used_at IS NULL AND expires_at>?`).get(hash(clean(d.token,200)),now());if(!row){fail(res,400,'Reset link is invalid or expired');return true}const passwordCheck=validatePassword(password);if(!passwordCheck.ok){fail(res,400,passwordCheck.reason);return true}db.prepare('UPDATE users SET password_hash=? WHERE id=?').run(passwordHash(password),row.user_id);db.prepare('UPDATE auth_tokens SET used_at=? WHERE id=?').run(now(),row.id);db.prepare('DELETE FROM sessions WHERE user_id=?').run(row.user_id);send(res,200,{ok:true});return true}
