...
async function api(req,res,url) {
  ...
  if(match&&req.method==='POST'&&match[2]==='send'){
    ...
    try{await sendInvoiceEmail(invoice,client,profile(user.tenant_id,user.organization_id));db.prepare(`UPDATE invoices SET status='sent',sent_at=? WHERE id=? AND tenant_id=? AND organization_id=?`).run(now(),invoice.id,user.tenant_id,user.organization_id);enterprise.audit(user,'sent','invoice',invoice.id,{to:client.email},req.socket.remoteAddress);return send(res,200,{ok:true});}catch(error){return fail(res,503,error.message);}
  }
