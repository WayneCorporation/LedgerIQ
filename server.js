...
function insertItems(invoiceId,items) { const stmt=db.prepare('INSERT INTO invoice_items(invoice_id,description,quantity,rate) VALUES(?,?,?,?)');for(const item of items){const description=clean(item.description,300),qty=Number(item.qty),rate=money(item.rate);if(!description||!Number.isFinite(qty)||qty<=0||Number.isNaN(rate))throw new Error('Every line item needs a valid description, quantity and rate');stmt.run(invoiceId,description,qty,rate);} }
