...
async function api(req,res,url) {
  ...
  if(req.method==='PUT'&&url.pathname==='/api/profile'){
    ...
    const d=await body(req),companyName=clean(d.companyName,120),ownerName=clean(d.ownerName,120),userEmail=email(d.email),currency=clean(d.currency,3),retentionDays=d.retentionDays===undefined?null:Number(d.retentionDays);
    ...
    db.prepare(`UPDATE tenants SET owner_name=?${retentionDays!==null?',data_retention_days=?':''} WHERE id=?`).run(...(retentionDays!==null?[ownerName,retentionDays,user.tenant_id]:[ownerName,user.tenant_id]));db.prepare(`UPDATE organizations SET name=?,legal_name=CASE WHEN legal_name='' THEN ? ELSE legal_name END,email=?,phone=?,registration_number=?,vat_number=?,address=?,currency=?,logo=? WHERE id=? AND tenant_id=?`).run(companyName,companyName,userEmail,clean(d.phone,40),clean(d.taxId,80),clean(d.vatNumber,80),clean(d.address,500),currency,clean(d.logo,700000),user.organization_id,user.tenant_id);enterprise.audit(user,'updated','organization',user.organization_id,{companyName},clientIp(req));return send(res,200,{profile:profile(user.tenant_id,user.organization_id)});
  }
