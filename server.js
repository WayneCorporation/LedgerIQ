...
CREATE TABLE IF NOT EXISTS quotes (
 id INTEGER PRIMARY KEY AUTOINCREMENT, tenant_id TEXT NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
 organization_id TEXT,
 number TEXT NOT NULL, client_id INTEGER NOT NULL REFERENCES clients(id), issue_date TEXT NOT NULL, expiry_date TEXT NOT NULL,
 status TEXT NOT NULL DEFAULT 'draft' CHECK(status IN ('draft','sent','accepted','declined','expired')),
 tax_rate REAL NOT NULL DEFAULT 0, discount REAL NOT NULL DEFAULT 0, notes TEXT NOT NULL DEFAULT '',
 payment_details TEXT NOT NULL DEFAULT '', sent_at TEXT, decided_at TEXT, converted_invoice_id INTEGER,
...
CREATE TABLE IF NOT EXISTS quote_items (
 ...
 description TEXT NOT NULL, quantity REAL NOT NULL, unit TEXT NOT NULL DEFAULT '', rate REAL NOT NULL
);
...
if(!tenantColumns.includes('data_retention_days'))db.exec("ALTER TABLE tenants ADD COLUMN data_retention_days INTEGER NOT NULL DEFAULT -1");
if(!tenantColumns.includes('cancelled_at'))db.exec('ALTER TABLE tenants ADD COLUMN cancelled_at TEXT');
...
function quoteRows(tenantId,organizationId) { const rows=db.prepare(`SELECT id,number,client_id clientId,issue_date issue,expiry_date expiry,status,tax_rate taxRate,discount,notes,payment_details paymentDetails,sent_at sentAt,decided_at decidedAt,converted_invoice_id convertedInvoiceId FROM quotes WHERE tenant_id=? AND organization_id=? ORDER BY issue_date DESC,id DESC`).all(tenantId,organizationId);const items=db.prepare('SELECT description,quantity qty,unit,rate FROM quote_items WHERE quote_id=? ORDER BY id');return rows.map(r=>({...r,items:items.all(r.id)})); }
function insertQuoteItems(quoteId,items) { const stmt=db.prepare('INSERT INTO quote_items(quote_id,description,quantity,rate,unit) VALUES(?,?,?,?,?)');for(const item of items){const description=clean(item.description,300),qty=Number(item.qty),rate=money(item.rate),unit=clean(item.unit,30);if(!description||!Number.isFinite(qty)||qty<=0||Number.isNaN(rate))throw new Error('Every line item needs a valid description, quantity and rate');stmt.run(quoteId,description,qty,rate,unit);} }
function quoteTotal(quote) { const subtotal=quote.items.reduce((s,i)=>s+Number(i.qty)*Number(i.rate),0);const discounted=Math.max(0,subtotal-Number(quote.discount||0));return discounted*(1+Number(quote.taxRate||0)/100); }
...
const enterprise=initEnterprise(db,{send,fail,body,clean,now,id,email,hash,operationalState,clientIp});
...
async function api(req,res,url) {
  ...
  if(process.env.NODE_ENV==='test'&&req.method==='POST'&&url.pathname==='/api/test/run-maintenance'){await runMaintenance();return send(res,200,{ok:true});}
  ...
  const protectedWrite=['/api/clients','/api/invoices','/api/quotes','/api/expenses','/api/enterprise'].some(prefix=>url.pathname===prefix||url.pathname.startsWith(prefix+'/'))&&['POST','PUT','PATCH','DELETE'].includes(req.method);
  ...
  if(req.method==='PUT'&&url.pathname==='/api/profile'){
    ...
    const d=await body(req),companyName=clean(d.companyName,120),ownerName=clean(d.ownerName,120),userEmail=email(d.email),currency=clean(d.currency,3),templateId=clean(d.templateId,20)||'classic',accentColor=clean(d.accentColor,7)||'#5b5fef',font=clean(d.font,10)||'sans',retentionDays=d.retentionDays===undefined?null:Number(d.retentionDays);
    ...
    if(!INVOICE_TEMPLATES.includes(templateId)||!/^#[0-9a-f]{6}$/i.test(accentColor)||!INVOICE_FONTS.includes(font))return fail(res,400,'Invalid invoice template settings');
    if(retentionDays!==null&&!RETENTION_DAY_OPTIONS.includes(retentionDays))return fail(res,400,'Invalid data retention setting');
    db.prepare(`UPDATE tenants SET owner_name=?${retentionDays!==null?',data_retention_days=?':''} WHERE id=?`).run(...(retentionDays!==null?[ownerName,retentionDays,user.tenant_id]:[ownerName,user.tenant_id]));db.prepare(`UPDATE organizations SET name=?,legal_name=CASE WHEN legal_name='' THEN ? ELSE legal_name END,email=?,phone=?,registration_number=?,vat_number=?,address=?,currency=?,logo=?,invoice_template_id=?,invoice_accent_color=?,invoice_font=? WHERE id=? AND tenant_id=?`).run(companyName,companyName,userEmail,clean(d.phone,40),clean(d.taxId,80),clean(d.vatNumber,80),clean(d.address,500),currency,clean(d.logo,700000),templateId,accentColor,font,user.organization_id,user.tenant_id);enterprise.audit(user,'updated','organization',user.organization_id,{companyName},clientIp(req));return send(res,200,{profile:profile(user.tenant_id,user.organization_id)});
  }
  ...
  if(req.method==='POST'&&url.pathname==='/api/quotes'){
    ...
    if(!number||!issue||!expiry||expiry<issue||!client||!Array.isArray(d.items)||!d.items.length||!Number.isFinite(taxRate)||taxRate<0||taxRate>100||Number.isNaN(discount))return fail(res,400,'Check the quote details');
    try{const quoteId=transaction(()=>{const result=db.prepare(`INSERT INTO quotes(tenant_id,organization_id,number,client_id,issue_date,expiry_date,tax_rate,discount,notes,payment_details,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).run(user.tenant_id,user.organization_id,number,client.id,issue,expiry,taxRate,discount,clean(d.notes,1000),clean(d.paymentDetails,1000),now());insertQuoteItems(Number(result.lastInsertRowid),d.items);return Number(result.lastInsertRowid)});enterprise.audit(user,'created','quote',quoteId,{number,total:d.items.reduce((s,i)=>s+Number(i.qty)*Number(i.rate),0)},clientIp(req));return send(res,201,{id:quoteId});}catch(error){return fail(res,400,error.message.includes('UNIQUE')?'Quote number already exists':error.message);}
  }
  match=url.pathname.match(/^\/api\/quotes\/(\d+)(?:\/(send|convert-to-invoice))?$/);
  if(match&&req.method==='PUT'&&!match[2]){if(!enterprise.requirePermission(user,res,'manage_finance'))return;const d=await body(req),number=clean(d.number,50),issue=date(d.issue),expiry=date(d.expiry),client=db.prepare('SELECT id FROM clients WHERE id=? AND tenant_id=? AND organization_id=?').get(Number(d.clientId),user.tenant_id,user.organization_id),taxRate=Number(d.taxRate||0),discount=money(d.discount||0),existing=db.prepare('SELECT id FROM quotes WHERE id=? AND tenant_id=? AND organization_id=?').get(match[1],user.tenant_id,user.organization_id);if(!existing)return fail(res,404,'Quote not found');if(!number||!issue||!expiry||expiry<issue||!client||!Array.isArray(d.items)||!d.items.length||!Number.isFinite(taxRate)||taxRate<0||taxRate>100||Number.isNaN(discount))return fail(res,400,'Check the quote details');try{transaction(()=>{db.prepare(`UPDATE quotes SET number=?,client_id=?,issue_date=?,expiry_date=?,tax_rate=?,discount=?,notes=?,payment_details=? WHERE id=? AND tenant_id=? AND organization_id=?`).run(number,client.id,issue,expiry,taxRate,discount,clean(d.notes,1000),clean(d.paymentDetails,1000),match[1],user.tenant_id,user.organization_id);db.prepare('DELETE FROM quote_items WHERE quote_id=?').run(match[1]);insertQuoteItems(Number(match[1]),d.items)});enterprise.audit(user,'updated','quote',match[1],{number},clientIp(req));return send(res,200,{ok:true});}catch(error){return fail(res,400,error.message.includes('UNIQUE')?'Quote number already exists':error.message);}}
  if(match&&req.method==='PATCH'&&!match[2]){if(!enterprise.requirePermission(user,res,'manage_finance'))return;const d=await body(req),status=clean(d.status,10);if(!['draft','sent','accepted','declined','expired'].includes(status))return fail(res,400,'Invalid status');db.prepare(`UPDATE quotes SET status=?,decided_at=CASE WHEN ? IN ('accepted','declined') THEN ? ELSE decided_at END WHERE id=? AND tenant_id=? AND organization_id=?`).run(status,status,now(),match[1],user.tenant_id,user.organization_id);enterprise.audit(user,'status_changed','quote',match[1],{status},clientIp(req));return send(res,200,{ok:true});}
  if(match&&req.method==='DELETE'&&!match[2]){if(!enterprise.requirePermission(user,res,'manage_finance'))return;db.prepare('DELETE FROM quotes WHERE id=? AND tenant_id=? AND organization_id=?').run(match[1],user.tenant_id,user.organization_id);enterprise.audit(user,'deleted','quote',match[1],{},clientIp(req));return send(res,200,{ok:true});}
  if(match&&req.method==='POST'&&match[2]==='send'){
    if(!enterprise.requirePermission(user,res,'manage_finance'))return;const quote=quoteRows(user.tenant_id,user.organization_id).find(q=>q.id===Number(match[1]));if(!quote)return fail(res,404,'Quote not found');const client=db.prepare('SELECT * FROM clients WHERE id=? AND tenant_id=? AND organization_id=?').get(quote.clientId,user.tenant_id,user.organization_id);if(!client.email)return fail(res,400,'Add an email address to this client first');
    try{await sendQuoteEmail(quote,client,profile(user.tenant_id,user.organization_id));db.prepare(`UPDATE quotes SET status='sent',sent_at=? WHERE id=? AND tenant_id=? AND organization_id=?`).run(now(),quote.id,user.tenant_id,user.organization_id);enterprise.audit(user,'sent','quote',quote.id,{to:client.email},clientIp(req));return send(res,200,{ok:true});}catch(error){return fail(res,503,error.message);}
  }
  if(match&&req.method==='POST'&&match[2]==='convert-to-invoice'){
    if(!enterprise.requirePermission(user,res,'manage_finance'))return;const quote=quoteRows(user.tenant_id,user.organization_id).find(q=>q.id===Number(match[1]));if(!quote)return fail(res,404,'Quote not found');if(quote.convertedInvoiceId)return fail(res,409,'This quote was already converted to an invoice');const d=await body(req),number=clean(d.number,50);if(!number)return fail(res,400,'Provide the new invoice number');
    try{const invoiceId=transaction(()=>{const today=now().slice(0,10),due=new Date(`${today}T12:00:00Z`);due.setUTCDate(due.getUTCDate()+14);const result=db.prepare(`INSERT INTO invoices(tenant_id,organization_id,number,client_id,issue_date,due_date,tax_rate,discount,notes,payment_details,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).run(user.tenant_id,user.organization_id,number,quote.clientId,today,due.toISOString().slice(0,10),quote.taxRate,quote.discount,quote.notes,quote.paymentDetails,now());insertItems(Number(result.lastInsertRowid),quote.items);db.prepare(`UPDATE quotes SET converted_invoice_id=?,status='accepted',decided_at=? WHERE id=? AND tenant_id=? AND organization_id=?`).run(Number(result.lastInsertRowid),now(),match[1],user.tenant_id,user.organization_id);return Number(result.lastInsertRowid)});enterprise.audit(user,'converted','quote',match[1],{invoiceId},clientIp(req));return send(res,201,{id:invoiceId});}catch(error){return fail(res,400,error.message.includes('UNIQUE')?'Invoice number already exists':error.message);}
  }
...
async function purgeCancelledTenants(){const candidates=db.prepare(`SELECT id,name,cancelled_at,data_retention_days FROM tenants WHERE subscription_status='cancelled' AND cancelled_at IS NOT NULL AND data_retention_days>=0`).all();for(const tenant of candidates){const cutoff=new Date(tenant.cancelled_at).getTime()+Number(tenant.data_retention_days)*86400000;if(cutoff>Date.now())continue;try{await reportError(new Error('Retention purge: permanently deleting cancelled tenant data'),{tenantId:tenant.id,tenantName:tenant.name,cancelledAt:tenant.cancelled_at,retentionDays:tenant.data_retention_days})}catch{}db.prepare('DELETE FROM tenants WHERE id=?').run(tenant.id);}}
async function runMaintenance(){await enterprise.runJobs().catch(error=>console.error('Background job failed',error));db.prepare('DELETE FROM sessions WHERE expires_at<=?').run(now());db.prepare('DELETE FROM auth_tokens WHERE expires_at<?').run(now());db.prepare('DELETE FROM api_idempotency WHERE created_at<?').run(new Date(Date.now()-24*60*60*1000).toISOString());for(const [key,times] of rateBuckets)if(!times.some(t=>Date.now()-t<15*60_000))rateBuckets.delete(key);await purgeCancelledTenants().catch(error=>console.error('Retention purge failed',error));}
...
if(process.env.VERCEL){
  ...
  setInterval(()=>runMaintenance().catch(error=>console.error('Maintenance run failed',error)),60*60*1000).unref();
}
